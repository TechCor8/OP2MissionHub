...
namespace OP2MissionHub.Data
{
	...
	{
		public static bool IsTilePassable(Map map, Vector2Int tileXY)
		{
			// Out of bounds is not passable
			if (tileXY.x < 0 || tileXY.y < 0 || tileXY.x >= map.GetWidthInTiles() || tileXY.y >= map.GetHeightInTiles())
				return false;

			// Check for passable tile types
			CellType type = (CellType)map.GetCellType((ulong)tileXY.x, (ulong)tileXY.y);
			switch (type)
			{
		...
		public static bool IsAreaPassable(Map map, RectInt area)
		{
			for (int x=area.xMin; x < area.xMax; ++x)
			{
				for (int y=area.yMin; y < area.yMax; ++y)
				{
					if (!IsTilePassable(map, new Vector2Int(x,y)))
						return false;
				}
			}
		...
		{
			// Out of bounds is not passable
			if (tileXY.x < 0 || tileXY.y < 0 || tileXY.x >= cellTypeMap.width || tileXY.y >= cellTypeMap.height)
